  ...
  extends PriorityQueue<Segment<K, V>> implements RawKeyValueIterator {
	...
	LOG.info("MergeQ OnDiskData: adding: " + data.getPath());
        ...
        if (reducer < 0) {
          ...
          LOG.info("Creating segment w/ index " + data.getPath() + " reducer " + reducer);
          Path indexFile = new Path(data.getPath().toString() + ".index");
          SpillRecord sr = new SpillRecord(indexFile, new JobConf(conf));
          IndexRecord irec = sr.getIndex(reducer);
          LOG.info("Symlink index offset " + irec.startOffset + " + partLength " + irec.partLength);
          segments.add(new Segment<K, V>(conf, fs, data.getPath(),
